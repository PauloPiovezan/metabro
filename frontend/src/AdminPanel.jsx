...
//Effect to set a new Goal for that month
    useEffect(() => {
    ...
    const goalResponse = await fetch(`${API_URL}/goals`,{
        ...
        body: JSON.stringify({date: adminControl.date, goal: adminControl.goal})
        })
    ...
    return(
        <div className="flex flex-col bg-white rounded-2xl outline-black outline-1 w-100 p-4 items-center">
            ...
            <button className="bg-green-500 rounded-2xl active:bg-green-800 p-2 mt-4" onClick={() => {setAdminControl({...adminControl, goal:goalRef.current.value})}}> Definir Meta</button>
        </div>)
